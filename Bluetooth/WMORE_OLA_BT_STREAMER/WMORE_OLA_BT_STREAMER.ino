...
/* BlueTooth Globals */
const char* wmoreStreamingServiceUuid = "18ee1516-016b-4bec-a596-bcb96d160405";
const char* wmoreStreamingCharUuid = "18ee1516-016b-4bec-ad96-bcb96d160406";

BLEService wmoreStreamingService(wmoreStreamingServiceUuid);
BLEStringCharacteristic wmoreStreamingChar(wmoreStreamingCharUuid, BLERead | BLEBroadcast, STRING_SIZE);
...
// Commented out by Nathan. Using the old version. The only difference is the use of the "silent" variable in the newer one | Merge
...
// //Trigger Pin ISR // Commented out by Nathan
...
void writeSDBin(void) {
  ...
  {
    // Write binary data
    uint32_t recordLength = sensorDataFile.write(sdOutputData, SD_RECORD_LENGTH); // WMORE TODO add error checking and resolve hard-coded length        
...
void loop() {
  ...
  while (c) {
    if (timerIntFlag == true) { // Act if sampling timer has interrupted
      ...
      // Serial.print(sdOutputData); // Added in | Nathan
      ...
      // SerialPrintln(F("Please ensure the SD card is formatted correctly using https://www.sdcard.org/downloads/formatter/"));
      // if ((online.bluetooth == false) && (Central.connected())) {
      // //   // Central = BLE.central();
      // //   c = BLE.central();
...
void initBlueTooth() {
  ...
  if (!BLE.begin()) {
    ...
    SerialPrintln(F("Failed to initialize BLE!"));
    return;
  }
  BLE.setAdvertisedService(wmoreStreamingService);
  wmoreStreamingService.addCharacteristic(wmoreStreamingChar);

  BLE.addService(wmoreStreamingService);

  wmoreStreamingChar.writeValue("WMORE ready for streaming...\r\n");
  ...
  scanData.setLocalName("WMORE Logger Streamer 0");
